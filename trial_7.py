...
def overview_tab():
    ...
    df["Priority Level"] = df["priority_level"]
    df["Status"] = df["status"]
    df["Action"] = df["action"]
    df["Select"] = False

    project_options = df["Project"].dropna().str.title().unique().tolist()
    ...
    if priority_filter != "All":
        df = df[df["Priority Level"] == priority_filter]
    ...
    id_map = df["id"].tolist()
    df_editor = df[["Date of Message", "Message", "Project", "Action", "Deadline", "Priority Level", "Status", "Select"]].copy()
    ...
    for i in range(len(edited_df)):
        ...
        for task in tasks:
            if task["id"] == task_id:
                task["priority_level"] = row["Priority Level"]
