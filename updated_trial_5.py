...
def dashboard_tab():
    ...
    # Auto-escalate tasks within 7 days of deadline
    today = datetime.today().date()
    for task in user_tasks:
        if task.get("deadline"):
            try:
                deadline_date = datetime.strptime(task["deadline"], "%Y-%m-%d").date()
                if (deadline_date - today).days <= 7:
                    task["escalate"] = True
            except:
                pass
...
if st.session_state.role == "team_member":
    tab1, tab2, tab3 = st.tabs(["📊 Urgency Calculator", "🚨 High Priority Tasks", "📈 Progress Insights"])
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Urgency Calculator", "🚨 High Priority Tasks", "📈 Progress Insights", "👥 Team Dashboard"])
        "📊 Urgency Calculator",
        "🚨 High Priority Tasks",
        "📈 Progress Insights",
        "👥 Team Dashboard"
